...
draw-guest-notice = 你尚未登录。保存后，这幅画只会保存在此浏览器中。
draw-guest-save-confirm = 要把这幅画保存在此浏览器中吗？登录后可以从草稿中发布。
draw-guest-saved-title = 已保存在此浏览器中
draw-guest-saved = 这幅画只在此浏览器中，不在网站上。清除本站数据会删除它，在 Safari 中一周不访问也会被删除。登录后可以从草稿中发布，也可以先下载一份副本。
...
draw-upload-failed-lost = 无法发布这幅画，此浏览器也无法保存它。请重试，或下载 PNG。
drafts-local-heading = 此浏览器中的画作
drafts-local-note = 这些画作在发布前只保存在此浏览器中。
drafts-local-guest-note = 这些画作只保存在此浏览器中。登录或注册后即可发布。
drafts-local-post = 发布
drafts-local-delete-confirm = 要从此浏览器中删除这幅画吗？删除后无法恢复。
drafts-local-community-denied = 此账号无法在该社区发布。要作为个人画作发布吗？
drafts-local-post-failed = 无法发布这幅画。它仍保存在此浏览器中。
drafts-local-unavailable = 此浏览器无法保存画作，因此未登录时保存的画作不会显示在这里。
