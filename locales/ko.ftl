...
draw-guest-notice = 로그인하지 않았습니다. 저장하면 이 브라우저에만 보관됩니다.
draw-guest-save-confirm = 이 그림을 이 브라우저에 저장할까요? 로그인하면 임시 보관함에서 게시할 수 있습니다.
draw-guest-saved-title = 이 브라우저에 저장됨
draw-guest-saved = 이 그림은 사이트가 아니라 이 브라우저에만 있습니다. 사이트 데이터를 지우면 사라지고, Safari에서는 일주일 동안 방문하지 않아도 사라집니다. 로그인하면 임시 보관함에서 게시할 수 있고, 사본을 내려받아 둘 수도 있습니다.
...
draw-upload-failed-lost = 그림을 게시하지 못했고, 이 브라우저에는 보관할 수도 없습니다. 다시 시도하거나 PNG를 내려받으세요.
drafts-local-heading = 이 브라우저에 있는 그림
drafts-local-note = 이 그림들은 게시하기 전까지 이 브라우저에만 있습니다.
drafts-local-guest-note = 이 그림들은 이 브라우저에만 있습니다. 게시하려면 로그인하거나 가입하세요.
drafts-local-post = 게시하기
drafts-local-delete-confirm = 이 브라우저에서 그림을 삭제할까요? 되돌릴 수 없습니다.
drafts-local-community-denied = 이 계정으로는 그 커뮤니티에 게시할 수 없습니다. 개인 그림으로 게시할까요?
drafts-local-post-failed = 그림을 게시하지 못했습니다. 이 브라우저에는 그대로 남아 있습니다.
drafts-local-unavailable = 이 브라우저에는 그림을 보관할 수 없어서, 로그인하지 않고 저장한 그림은 여기 나오지 않습니다.
